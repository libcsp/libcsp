...
void rx_task(void * param) {
	...
	csp_iface_t *iface = &iface1;

	grlibCan_msg_t frame;
	grlibCan_dev_t *dev = device1;
...
int grlibCan_txWrapper(void * driver_data, uint32_t id, const uint8_t * data, uint8_t dlc) {
...
int main(int argc, char ** argv) {
	...
	csp_rtable_set(2, 32, &iface1, CSP_NO_VIA_ADDRESS);
	csp_rtable_print();
	/* Start router */
