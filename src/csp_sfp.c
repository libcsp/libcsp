	...
	*return_datasize = 0;
	...
	do {
		...
		if (data == NULL) {
			...
			if (data == NULL) {
				csp_print(CSP_LL_ERROR, "%s: %u:%u, malloc(%" PRIu32 ") failed\n", __func__, packet->id.src, packet->id.sport, datasize);
				csp_buffer_free(packet);
