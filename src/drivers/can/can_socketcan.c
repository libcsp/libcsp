...
static void * socketcan_rx_thread(void * arg)
{
	...
	while (1) {
		...
		if (nbytes < 0) {
			csp_log_error("%s[%s]: read() failed, errno %d: %s", __func__, ctx->name, errno, strerror(errno));
			continue;
		}

		if (nbytes != sizeof(frame)) {
			csp_log_warn("%s[%s]: Read incomplete CAN frame, size: %d, expected: %u bytes", __func__, ctx->name, nbytes, (unsigned int) sizeof(frame));
			continue;
		}
		...
		if (frame.can_id & (CAN_ERR_FLAG | CAN_RTR_FLAG)) {
			csp_log_warn("%s[%s]: discarding ERR/RTR/SFF frame", __func__, ctx->name);
			continue;
		}
...
static int csp_can_tx_frame(void * driver_data, uint32_t id, const uint8_t * data, uint8_t dlc)
{
	...
	while (write(ctx->socket, &frame, sizeof(frame)) != sizeof(frame)) {
		if ((errno != ENOBUFS) || (elapsed_ms >= 1000)) {
			csp_log_warn("%s[%s]: write() failed, errno %d: %s", __func__, ctx->name, errno, strerror(errno));
			return CSP_ERR_TX;
		}
		csp_sleep_ms(5);
		elapsed_ms += 5;
...
int csp_can_socketcan_open_and_add_interface(const char * device, const char * ifname, int bitrate, bool promisc, csp_iface_t ** return_iface)
{
	...
	if ((ctx->socket = socket(PF_CAN, SOCK_RAW, CAN_RAW)) < 0) {
		csp_log_error("%s[%s]: socket() failed, error: %s", __func__, ctx->name, strerror(errno));
		socketcan_free(ctx);
		return CSP_ERR_INVAL;
	}
	...
	if (ioctl(ctx->socket, SIOCGIFINDEX, &ifr) < 0) {
		csp_log_error("%s[%s]: device: [%s], ioctl() failed, error: %s", __func__, ctx->name, device, strerror(errno));
		socketcan_free(ctx);
		return CSP_ERR_INVAL;
	}
	...
	if (bind(ctx->socket, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
		csp_log_error("%s[%s]: bind() failed, error: %s", __func__, ctx->name, strerror(errno));
		socketcan_free(ctx);
		return CSP_ERR_INVAL;
	}
	...
	if (promisc == false) {
		...
		if (setsockopt(ctx->socket, SOL_CAN_RAW, CAN_RAW_FILTER, &filter, sizeof(filter)) < 0) {
			csp_log_error("%s[%s]: setsockopt() failed, error: %s", __func__, ctx->name, strerror(errno));
			socketcan_free(ctx);
			return CSP_ERR_INVAL;
		}
	}
	...
	if (res != CSP_ERR_NONE) {
		csp_log_error("%s[%s]: csp_can_add_interface() failed, error: %d", __func__, ctx->name, res);
		socketcan_free(ctx);
		return res;
	}
	...
	if (pthread_create(&ctx->rx_thread, NULL, socketcan_rx_thread, ctx) != 0) {
		csp_log_error("%s[%s]: pthread_create() failed, error: %s", __func__, ctx->name, strerror(errno));
...
int csp_can_socketcan_stop(csp_iface_t *iface)
{
	...
	if (error != 0) {
		csp_log_error("%s[%s]: pthread_cancel() failed, error: %s", __func__, ctx->name, strerror(errno));
		return CSP_ERR_DRIVER;
	}
	error = pthread_join(ctx->rx_thread, NULL);
	if (error != 0) {
		csp_log_error("%s[%s]: pthread_join() failed, error: %s", __func__, ctx->name, strerror(errno));
		return CSP_ERR_DRIVER;
	}
        socketcan_free(ctx);
	return CSP_ERR_NONE;
