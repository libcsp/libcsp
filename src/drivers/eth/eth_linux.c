...
#include <net/if.h>
#include <pthread.h>
#include <unistd.h> // For readlink, close
...
typedef struct {
	...
	volatile bool stop_rx_thread;
	uint8_t tx_buffer[CSP_ETH_BUF_SIZE];
	uint8_t rx_buffer[CSP_ETH_BUF_SIZE];
} eth_context_t;
...
int csp_eth_tx_frame(void * driver_data, csp_eth_header_t * eth_frame) {
	...
	if (sendto(ctx->sockfd, (void *)eth_frame, txsize, 0, (struct sockaddr *)&socket_address, sizeof(struct sockaddr_ll)) < 0) {
		// perror is not used here to avoid excessive printing in the TX path
...
void * csp_eth_rx_loop(void * param) {
	...
	csp_eth_header_t * eth_frame = (csp_eth_header_t *)ctx->rx_buffer;

	while (!ctx->stop_rx_thread) {

		/* Receive packet segment */
		ssize_t received_len = recvfrom(ctx->sockfd, ctx->rx_buffer, CSP_ETH_BUF_SIZE, 0, NULL, NULL);
		if (received_len < 0) {
			perror("recvfrom failed in rx_loop");
			// If recvfrom fails, the socket might be in an error state.
			// Depending on the error, we might want to break the loop.
			// For now, we continue, but in a real-world scenario, you might need more robust handling.
			continue;
		}
...
int csp_eth_init(const char * device, const char * ifname, int mtu, unsigned int node_id, bool promisc, csp_iface_t ** return_iface) {
	...
	ctx->sockfd = -1; // Initialize to invalid value

	strncpy(ctx->name, ifname, CSP_IFLIST_NAME_MAX);
	ctx->name[CSP_IFLIST_NAME_MAX] = '\0'; // Ensure null termination

	ctx->ifdata.iface.name = ctx->name;
	ctx->ifdata.tx_func = &csp_eth_tx_frame;
	ctx->ifdata.tx_buf = (csp_eth_header_t *)&ctx->tx_buffer; // Use buffer from context
	...
	ctx->stop_rx_thread = false;

	/* Ether header 14 byte, seg header 4 byte, CSP header 6 byte */
	if (mtu < 24) {
		csp_print(CSP_LL_ERROR, "csp_if_eth_init: mtu < 24\n");
		free(ctx); // No resources acquired yet, so direct free is fine
	...
	if ((ctx->sockfd = socket(AF_PACKET, SOCK_RAW, htobe16(CSP_ETH_TYPE_CSP))) == -1) {
		perror("socket");
		char exe[1024];
		int count = readlink("/proc/self/exe", exe, sizeof(exe) - 1);
		if (count > 0) {
			exe[count] = '\0';
			csp_print(CSP_LL_ERROR, "Use command 'sudo setcap cap_net_raw+ep %s'\n", exe);
		}
		goto error_exit;
	}
	...
	csp_print(CSP_LL_ERROR,
			  "INIT %s %s idx %d node %d mac %02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx\n",
	...
	int sockopt = 1; // Initialize to 1 to enable the option
	...
	if (pthread_create(&ctx->rx_thread_handle, &attr, &csp_eth_rx_loop, ctx) != 0) {
...
error_exit:
	...
	return CSP_ERR_DRIVER; // Return a general driver error
}
