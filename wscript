...
def options(ctx):
    ...
    gr.add_option('--enable-if-tnc', action='store_true', help='Enable TNC over KISS/RS.232 interface')
    gr.add_option('--enable-if-ip', action='store_true', help='Enable sending over the network')
...
def configure(ctx):
    ...
    if ctx.options.enable_if_tnc:
        ctx.env.append_unique('FILES_CSP', 'src/interfaces/csp_if_tnc.c')
    if ctx.options.enable_if_ip:
        ctx.env.append_unique('FILES_CSP', 'src/interfaces/csp_if_ip.c')
...
def build(ctx):
    ...
    if ctx.options.install_csp:
        ...
        if 'src/interfaces/csp_if_tnc.c' in ctx.env.FILES_CSP:
            ctx.install_files('${PREFIX}/include/csp/interfaces', 'include/csp/interfaces/csp_if_tnc.h')
        if 'src/interfaces/csp_if_ip.c' in ctx.env.FILES_CSP:
            ctx.install_files('${PREFIX}/include/csp/interfaces', 'include/csp/interfaces/csp_if_ip.h')
